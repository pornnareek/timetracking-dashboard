// api/data.js — Vercel Serverless Function (Multi-Year)
const WORKING_DAYS = {
...
export default async function handler(req, res) {
  ...
  // ── ดึงข้อมูลทุกปีตั้งแต่ startYear จนถึงปัจจุบัน ──
  let allEntries=[];
  ...
  for(const entry of allEntries){
    ...
    if(!personMap[rawName]) personMap[rawName]={name:rawName,dept,avatar:entry.user?.profilePicture||null,email:entry.user?.email||"",monthly:{},yearly:{}};
  ...
  // ── สร้าง months list ตั้งแต่ startYear จนถึงเดือนปัจจุบัน ──
  const now=new Date();
  const months=[];
  ...
  const persons=Object.values(personMap).map(p=>{
    ...
    for(const y of years){
      ...
      yearlyData[y]={ms,display:formatDuration(ms),pct:Math.round(pct*100),status:pct>=0.6?"ok":pct>=0.3?"warn":"bad"};
    }
